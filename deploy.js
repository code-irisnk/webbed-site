...
fs.writeFile('commit-sha', commitSha, 'utf8')
    .then(() => console.log('Commit SHA written to file'))
    .catch(error => console.error(`Error writing commit SHA to file: ${error.message}`));
...
async function processFile(file) {
    try {
        ...
        if (htmlContent.includes('{{commitSha}}')) {
            ...
            const result = template({ commitSha });
...
fs.readdir('.')
    ...
    .catch(error => console.error(`Error reading directory: ${error.message}`));
